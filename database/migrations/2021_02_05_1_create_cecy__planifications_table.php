...
class CreateCecyPlanificationsTable extends Migration
{
    ...
    public function up()
    {
        Schema::connection('pgsql-cecy')->create('planifications', function (Blueprint $table) {
            ...
            $table->foreignId('teacher_id')->constrained('authentication.users')->comment('docente encargado de realizar toda la planificación del cecy, crear su equipo de docentes a impartir el curso, entregar attendance, asistencia, etc'); //responsable_id
            //$table->foreignId('school_period_id')->constrained('ignug.school_periods')->nullable(); //school_period_id
            $table->foreignId('status_id')->constrained('app.catalogues')->comment('Una planifición puede ser dada de baja(cuando fue propuesta por el coodinador pero no se ejecuto), en ejecución o completada(entrego attendance y el resto de documentación)');
            $table->timestamps();
            $table->softDeletes();
        });
    }
